...
"psql -c \"CREATE USER instark WITH SUPERUSER PASSWORD 'instark'\""

lxc exec $CONTAINER -- su - postgres -c \
"psql -c \"CREATE DATABASE instark WITH OWNER instark\""

echo "Install virtualenv..."

lxc exec $CONTAINER -- apt install python3-venv -y
lxc exec $CONTAINER -- su - $USER -c "python3 -m pip install -U pip wheel"
lxc exec $CONTAINER -- su - $USER -c "python3 -m venv /opt/instark/env"

echo "Link project to home..."

lxc exec $CONTAINER -- ln -s $PROJECT_PATH /opt/$USER/instark

echo "Install virtualenv dependencies..."

lxc exec $CONTAINER -- su - $USER -c "/opt/instark/env/bin/python -m pip \
install -U pip wheel"
lxc exec $CONTAINER -- su - $USER -c "/opt/instark/env/bin/python -m pip \
install -r /opt/instark/instark/requirements.txt"
